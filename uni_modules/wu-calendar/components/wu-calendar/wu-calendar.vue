<template>
	<view class="wu-calendar" @click.stop>
		...
		<view v-if="insert || show" class="wu-calendar__content"
			...
			<slot name="header" :nowDate="nowDate">
				<view class="wu-calendar__header">
					...
					<template v-if="slideSwitchMode == 'vertical'">
						...
						<text v-if="isToday" class="wu-calendar__backtoday vertical" @click="backToday">{{todayText}}</text>
					</template>
					<!-- 横向滑动与无滑动展示内容 -->
					<template v-else>
						<view class="left-btn-reset" v-if="isShowBtn || singleReset">
							<button @click="resetValue" size="mini" style="color:#ffffff;backgroundColor:#f9ae3d;borderColor:#1AAD19">重置</button>
						</view>
						<view class="right-btn-confirm" v-if="isShowBtn">
							<button @click="confirmValue" size="mini" style="color:#ffffff;backgroundColor:#2C81FD;borderColor:#1AAD19">确认</button>
						</view>
						...
						<text v-if="isToday" class="wu-calendar__backtoday" @click="backToday">{{todayText}}</text>
					</template>
				</view>
			</slot>
			...
			<view class="wu-calendar__fold" v-if="type !== 'year' && Fold && showFlod" @click="FoldClick">
...
<script>
	...
	export default {
		...
		emits: ['close', 'cancel', 'confirm', 'change', 'monthSwitch', 'foldSwitch','confirmRange','resetRange'],
		data() {
			return {
				show: false,
				weeks: [],
