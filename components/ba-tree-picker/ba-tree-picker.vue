...
<template>
  <view>
    <view class="tree-cover" :class="{'show':showDialog}" @click="_cancel"></view>
    <view class="tree-dialog" :class="{'show':showDialog}">
      <view class="tree-bar">
        <view class="tree-bar-cancel" :style="{'color':cancelColor}" hover-class="hover-c" @click="_cancel">取消
        </view>
          ...
          <block v-for="(item, index) in treeList" :key="index">
            <view class="tree-item" :style="[{
              ...
              <view class="item-label">
                <view class="item-icon uni-inline-item" @click.stop="_onItemSwitch(item, index)">
                ...
                <view class="uni-flex-item uni-inline-item" @click.stop="_onItemSelect(item, index)">
...
export default {
  ...
  props: {
    ...
    selectedDataType: { //选中后的数据类型。'parentAndChild':返回选中的父级和父级下的所有子集,'parent'：只返回父级,'child'：只返回子集
      type: String,
      default: 'parent'
    },
  },
  ...
  methods: {
    ...
    _formatTreeData(list = [], level = 0, parentItem, isShowChild = true) {
      ...
      list.forEach(item => {
        ...
        initCheckStatus = item.initCheckStatus ? item.initCheckStatus : initCheckStatus;
        let itemT = {
        ...
        /*回显*/
        // 如果当前项是父项且有子项
        let tempChildCheckCount = 0
        if (itemT.children && itemT.children.length > 0) {
          // 检查子项的选中情况
          tempChildCheckCount = 0;
          itemT.children.forEach(child => {
            if (this.selectedData.indexOf(child[this.valueKey]) >= 0) {
              tempChildCheckCount++;
            }
          });

          // 根据子项的选中情况设置父项的状态
          if (tempChildCheckCount === itemT.children.length) {
            itemT.checkStatus = 2; // 所有子项选中，父项选中
          } else if (tempChildCheckCount > 0) {
            itemT.checkStatus = 1; // 部分子项选中，父项半选中
          } else {
            itemT.checkStatus = 0; // 没有子项选中，父项未选中
          }
        }
    ...
    _onItemSelect(item, index) {
      ...
      //解决需要返回选中的父节点及所有子节点时只返回父节点的情况
      if (this.multiple && item.children && item.children.length > 0 && item.level >= this.treeList[index].level) {
    ...
    _onItemChildSelect(item, index) {
      ...
      if (item.childCount && item.childCount > 0) {
        ...
        while (index < this.treeList.length && this.treeList[index].level > item.level) {
          ...
          //解决需要返回选中的父节点及所有子节点时只返回父节点的情况
          this._onItemSwitch(itemChild, index - 1)
