<template>
  <view class="easy-scorll">
    <scroll-view
      :scroll-y="true"
      class="nav-list"
      :scroll-into-view="'scroll-nav-' + contentIndex"
    >
      <view
        class="nav-item"
    ...
    <view class="list-wrapper">
      ...
      >
        <!-- 内容部分没有使用插槽的原因是微信小程序端不适配 -->
        <view
          class="list"
          v-for="(item, listIndex) of list"
          :key="listIndex"
          :id="'scroll-item-' + listIndex"
        >
          <view class="title"
            ><text>分类{{ listIndex + 1 }}-对应内容</text></view
          >
          <view class="item-container">
            <view
              class="item"
              v-for="(item, index) of item.itemList"
              :key="index"
            >
...
export default {
  ...
  methods: {
    init() {
      ...
      // 初始定位左侧高亮位置
      this.viewNavIndex = this.clickedNavIndex = this.list.findIndex(
        (item) => item[this.itemKey] === this.curNavKey
      );
